...
## Extracting WMH volumes from segmented images

The script `MARS-WMH_extract_volumes.py` allows you to extract and aggregate the voxel count, volume and cluster count of the segmented WMH from one or more NIfTI files corresponding to MARS segmentation label-maps.  
...
Run the script in your local Python environment or use Apptainer and run the script within the MARS-WMH container with its Python installation, e.g.:  
`apptainer exec -B /path/to/segmented/data mars-wmh-nnunet.sif python MARS-WMH_extract_volumes.py /path/to/segmented/data`
